...
void ArenaCameraNode::initialize_()
{
  ...
  auto shutdown_flag = shutting_down_flag_;
  m_pSystem = std::shared_ptr<Arena::ISystem>(
      nullptr, [shutdown_flag](Arena::ISystem* pSystem) {
        if (!pSystem) {
          return;
        }
        // Skip SDK teardown during process shutdown to avoid known crash path.
  ...
  m_pDevice = std::shared_ptr<Arena::IDevice>(
      nullptr, [this, shutdown_flag](Arena::IDevice* pDevice) {
        if (!pDevice || !m_pSystem) {
          return;
        }
        // Skip SDK teardown during process shutdown to avoid known crash path.
        if (shutdown_flag && shutdown_flag->load()) {
...
void ArenaCameraNode::wait_for_device_timer_callback_()
{
  ...
  // at least on is found
    ...
    try {
      ...
      log_warn(std::string("Arena/GenICam exception while starting stream: ") +
               e.what());
      log_warn("Will retry device initialization...");
    } catch (std::exception& e) {
      log_warn(std::string("Exception while starting stream: ") + e.what());
      log_warn("Will retry device initialization...");
    }
  }
}
...
void ArenaCameraNode::publish_images_()
{
  ...
  while (rclcpp::ok()) {
    try {
      ...
      pImage = m_pDevice->GetImage(10000);
      uint64_t timestamp = pImage->GetTimestamp();
      uint64_t frame_id = pImage->GetFrameId();

      // Print the timestamp
      // std::cout << "Image timestamp (nanoseconds): " << timestamp << std::endl;
      // std::cout << "Image Frame ID: " << frame_id << std::endl;
      msg_form_image_(pImage, *p_image_msg);

      m_pub_->publish(std::move(p_image_msg));

      log_info(std::string("image timestamp: ") + std::to_string(timestamp));
...
void ArenaCameraNode::set_nodes_()
{
  ...
  set_nodes_custom_params_();
  // set_nodes_gpio_trigger_();
...
void ArenaCameraNode::set_nodes_custom_params_()
{
  log_info("\tSetting Custom params...");
  auto nodemap = m_pDevice->GetNodeMap();
  Arena::SetNodeValue<int64_t>(nodemap, 
    "DeviceLinkThroughputReserve",
    15);
    log_info("here");
  Arena::SetNodeValue<bool>(nodemap, 
     "AcquisitionFrameRateEnable",
     true);
     log_info("here2");
  Arena::SetNodeValue<double>(nodemap, 
     "AcquisitionFrameRate",
     1.0);
     log_info("here3");
  log_info("did throughput");
  Arena::SetNodeValue<int64_t>(nodemap, 
    "GevSCPD",
    80);
  Arena::SetNodeValue<int64_t>(nodemap,
    ...
    90);
  Arena::SetNodeValue<int64_t>(nodemap, 
    "PacketResendWindowFrameCount",
    4);
  log_info("\tDone setting params");


}

void ArenaCameraNode::set_nodes_decimation_()
{
  log_info("\tSetting Decimation...");
  auto nodemap = m_pDevice->GetNodeMap();
  ...
  Arena::SetNodeValue<int64_t>(nodemap,
	...
	Arena::SetNodeValue<int64_t>(nodemap,
		...
		2);
  log_info("\tSensor Decimation set to 2x2");
}
...
void ArenaCameraNode::set_nodes_gpio_trigger_()
{
  auto nodemap = m_pDevice->GetNodeMap();
  // Enable trigger mode before setting the source and selector
    ...
    auto msg =
        std::string(
            "\ttrigger_mode is activated on line 2.");
    log_warn(msg);
