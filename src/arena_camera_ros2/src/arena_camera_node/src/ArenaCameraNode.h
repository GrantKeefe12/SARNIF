...
class ArenaCameraNode : public rclcpp::Node
{
 public:
  ...
  ~ArenaCameraNode()
  {
    ...
    if (shutting_down_flag_) {
      shutting_down_flag_->store(true);
    }
    ...
    // Trigger SDK deleters while node object is still alive.
    m_pDevice.reset();
    m_pSystem.reset();
  }
 ...
 private:
  std::shared_ptr<std::atomic_bool> shutting_down_flag_ =
  ...
  void set_nodes_gpio_trigger_();
  void set_nodes_test_pattern_image_();
  void set_nodes_decimation_();
  void set_nodes_custom_params_();
